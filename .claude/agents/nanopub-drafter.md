...
5. **Fill in the draft** in `nanopubs/drafts/`. Each draft already lists every field, in form order, inside a `<!-- FIELDS:GENERATED -->` region generated from the template snapshot — **do not edit inside those markers or add/rename fields**; that region is the authoritative field list and is regenerated by `pixi run -e tests gen-drafts`. Put each field's value in its fenced block (required: a value; optional: a value or `*(skip — optional)*`), and read the draft's **Field notes** section for the per-field guidance (caps, mappings, which URI to pull from `PUBLISHED.md`). If a field you expected is missing from the generated region, the snapshot is stale — stop and flag it, don't invent the field.
6. The generated region already carries the template URI and the full field list, so no separate paste is needed — the user verifies alignment against that region.
